...
fn device_row(
    ...
    ui.painter().rect(rect, CornerRadius::same(0), bg, stroke, egui::StrokeKind::Inside);
    ...
    let av_col = match &peer.kind {
        PeerKind::Remote    => p.accent2,
        PeerKind::Bluetooth(_) => p.warn,
        PeerKind::Local     => if is_recent { p.accent } else { tint(p.accent, 180) },
    };
    ...
    let sub = if is_remote { "Remote device".to_string() }
              else if matches!(peer.kind, PeerKind::Bluetooth(_)) {
                  peer.bt_addr.as_deref().unwrap_or("Bluetooth").to_string()
              }
    ...
    if !sub.is_empty() {
        ui.painter().text(
            ...
            egui::FontId::proportional(10.0), p.text_faint);
    }

    // ── Right side ─────────────────────────────────────────────────────────
    let addr_str = match &peer.kind {
        PeerKind::Bluetooth(bt) => bt.clone(),
        _ => peer.addr.to_string(),
    };
    ui.painter().text(
        egui::pos2(content.right() - 160.0, content.center().y),
        egui::Align2::LEFT_CENTER, &addr_str,
        egui::FontId::proportional(11.0), p.text_dim);

    let lat_str = match peer.latency {
        Some(ms) => format!("{} ms", ms),
        None     => if matches!(peer.kind, PeerKind::Bluetooth(_)) { "BT".to_string() } else { "—".to_string() },
    };
    ...
    ui.painter().text(
        ...
        egui::Align2::LEFT_CENTER, &lat_str,
        egui::FontId::proportional(11.0), lat_col);

    // Type badge
    let (type_lbl, type_col) = match &peer.kind {
        PeerKind::Remote        => ("Remote", p.accent2),
        PeerKind::Bluetooth(_)  => ("BT",     p.warn),
        PeerKind::Local         => ("Local",  p.success),
    };
...
struct Pal {
    ...
    success: Color32, danger: Color32, warn: Color32, pro: Color32,
}
impl Pal {
    fn dark() -> Self {
        Self {
...
impl HistoryEntry {
    fn time_display(&self) -> String {
        ...
        else                  { format!("{} wks ago",  diff / 604800) }
    }
...
fn load_history() -> Vec<HistoryEntry> {
    ...
    for line in text.lines().skip(1) {
        ...
        out.push(HistoryEntry {
            ...
            success: ok, error: err, file_path: fpath,
        });
    }
...
fn append_history(entry: &HistoryEntry) {
    ...
    if let Ok(mut f) = fs::OpenOptions::new().create(true).append(true).open(&path) {
        ...
        let dir        = if entry.direction   == TransferDir::Sent  { "sent"   } else { "received" };
        let trans_type = if entry.transfer_type == TransferType::Local { "local" } else { "remote"  };
        let err  = entry.error.as_deref().unwrap_or("").replace(',', ";");
        let fpath = entry.file_path.as_ref()
            .map(|p| p.to_string_lossy().to_string()).unwrap_or_default().replace(',', ";");
        let ok = if entry.success { 1 } else { 0 };
        let _ = writeln!(f, "{},{},{},{},{},{},{},{},{}",
            entry.timestamp, dir, trans_type, entry.file_name,
            entry.file_size, entry.peer_name, ok, err, fpath);
    }
}
...
impl QueueItem {
    fn new(path: PathBuf) -> Self {
        ...
        Self { path, name, size, progress: None, error: None }
    }
    fn is_done(&self)    -> bool { self.progress == Some(1.0) }
    fn is_failed(&self)  -> bool { self.error.is_some() }
    fn is_pending(&self) -> bool { self.progress.is_none() && self.error.is_none() }
    fn is_active(&self)  -> bool { matches!(self.progress, Some(p) if p >= 0.0 && p < 1.0) && self.error.is_none() }
}
...
#[derive(Clone, Debug, PartialEq)]
enum PeerKind {
    Local,              // discovered via UDP broadcast on LAN
    Remote,             // manually added or relay
    Bluetooth(String),  // BT MAC address, e.g. "AA:BB:CC:DD:EE:FF"
}
...
struct Peer {
    ...
    bt_addr: Option<String>, // Bluetooth MAC address (if available)
}

#[derive(Clone, Debug, PartialEq)]
enum Tab { Scan, Send, History, Sync, Settings }
...
/// Live progress of the currently-incoming file, updated from the receive thread.
...
struct IncomingTransfer {
    ...
    received_bytes:    u64,
    peer_name:         String,
    batch_file_index:  u32,  // 1-based (1 = first file in the batch)
    batch_total_files: u32,  // 0 means unknown/single
...
struct RecvState {
    ...
    auto_open_folder:  bool,
    save_dir:          Option<PathBuf>,
    /// Set while a file is being received; cleared when done.
    active_incoming:   Option<IncomingTransfer>,
}
...
pub struct App {
    ...
    remote_ip_buf:   String,
    remote_name_buf: String,
    remote_msg:      Option<(String, bool)>,
    ...
    is_relay_mode:          bool,
    relay_sync_map:         std::collections::HashMap<String, PathBuf>,
    ...
    // ── Bluetooth ─────────────────────────────────────────────────────────
    bt_scan_rx:   Option<std::sync::mpsc::Receiver<Vec<Peer>>>,
    bt_scan_done: bool,
}
        ...
        Self {
            ...
            manual_peers: prefs.manual_peers.iter().filter_map(|(name, addr_str)| {
                addr_str.parse::<std::net::IpAddr>().ok().map(|addr| Peer {
                    ...
                    kind: PeerKind::Remote, latency: None, bt_addr: None,
                })
            }).collect(),
            latency_rx:       None,
...
impl App {
    ...
    fn start_scan(&mut self) {
                    ...
                    if let Ok((n, addr)) = sock.recv_from(&mut buf) {
                        ...
                        if let Some(name) = msg.strip_prefix(PEER_PREFIX) {
                            ...
                            if !found.iter().any(|p| p.addr == ip) {
                                found.push(Peer { name, addr: ip, kind: PeerKind::Local, latency: None, bt_addr: None });
                            }
                        }
                    }
                }
        ...
        // ── Bluetooth scan ────────────────────────────────────────────────
        let (bt_tx, bt_rx) = std::sync::mpsc::channel::<Vec<Peer>>();
        self.bt_scan_rx = Some(bt_rx);
        thread::spawn(move || {
            let bt_peers = scan_bluetooth_peers();
            let _ = bt_tx.send(bt_peers);
        });
    }
    ...
    fn remove_queue_item(&mut self, idx: usize) {
        if idx < self.queue.len() { self.queue.remove(idx); }
    }
    ...
    fn retry_failed(&mut self) {
        for item in &mut self.queue {
            if item.is_failed() { item.error = None; item.progress = None; }
        }
    }

    fn start_send_queue(&mut self) {
        let Some(peer) = self.selected.and_then(|i| self.peers.get(i)).cloned() else { return; };
        ...
        let relay_stream_opt = if use_relay {
            if let Some(stream_arc) = &self.relay_stream {
                if let Ok(mut guard) = stream_arc.lock() { guard.take() } else { None }
            } else { None }
        } else { None };

        for item in &mut self.queue {
            if item.is_pending() { item.progress = Some(0.0); }
        }

        let items: Vec<(usize, PathBuf, String, u64)> = self.queue.iter().enumerate()
            .filter(|(_, q)| q.is_active() || q.progress == Some(0.0))
            .map(|(i, q)| (i, q.path.clone(), q.name.clone(), q.size))
            .collect();

        // Compute batch totals for progress reporting on the receiver side
        let total_files = items.len() as u32;
        let total_bytes: u64 = items.iter().map(|(_, _, _, sz)| sz).sum();

        let (tx, rx) = std::sync::mpsc::channel::<QueueMsg>();
        self.queue_rx = Some(rx);
        let peer_name = peer.name.clone();
        let peer_bt_addr = peer.bt_addr.clone();
        ...
        thread::spawn(move || {
            for (seq_idx, (index, path, name, size)) in items.into_iter().enumerate() {
                let tx2  = tx.clone();
                let pn   = peer_name.clone();
                let nm   = name.clone();
                // batch_info: (file_index 1-based, total_files, total_bytes)
                let batch_info = Some((seq_idx as u32 + 1, total_files, total_bytes));

                let result = if use_relay_clone {
                    if let Some(stream_arc) = &shared_stream {
                        if let Ok(mut guard) = stream_arc.lock() {
                            if let Some(mut stream) = guard.take() {
                                let r = send_file_via_relay(&path, &name, size, &mut stream, move |p| {
                                    let _ = tx2.send(QueueMsg::Progress { index, progress: p });
                                });
                                r
                            } else { Err("No relay stream available".into()) }
                        } else { Err("Failed to lock relay stream".into()) }
                    } else { Err("No relay stream available".into()) }
                } else {
                    send_file_resumable(&path, &name, size, peer.addr, batch_info, peer_bt_addr.clone(), move |p| {
                        let _ = tx2.send(QueueMsg::Progress { index, progress: p });
                    })
                };
                ...
                match result {
                    Ok(()) => {
                        append_history(&HistoryEntry {
                            timestamp: ts, direction: TransferDir::Sent,
                            transfer_type: TransferType::Local, file_name: nm,
                            file_size: size, peer_name: pn, success: true, error: None,
                    ...
                    Err(e) => {
                        append_history(&HistoryEntry {
                            timestamp: ts, direction: TransferDir::Sent,
                            transfer_type: TransferType::Local, file_name: nm,
                            file_size: size, peer_name: pn, success: false,
                            error: Some(e.clone()), file_path: Some(path.clone()),
                        });
    ...
    fn start_sync_watcher(&mut self) {
        ...
        thread::spawn(move || {
            let mut job_mtimes: Vec<std::collections::HashMap<String, u64>> =
                jobs.iter().map(|_| std::collections::HashMap::new()).collect();
            ...
            loop {
                ...
                for (job_idx, job) in jobs.iter().enumerate() {
                    let Ok(entries) = fs::read_dir(&job.folder) else { continue; };
                    ...
                    for entry in entries.flatten() {
                        ...
                        let current_mtime = fs::metadata(&path)
                            ...
                            .unwrap_or(0);
                        if let Some(&last) = job_mtimes[job_idx].get(&key) {
                            if last >= current_mtime { continue; }
                        }
                        let name = path.file_name().unwrap_or_default().to_string_lossy().to_string();
                        let size = fs::metadata(&path).map(|m| m.len()).unwrap_or(0);
                    ...
                    for (path, key, name, size, current_mtime) in files_to_send {
                        ...
                        match send_file_sync(&path, &name, size, current_mtime, job.peer_addr) {
                            Ok(SyncResult::Sent)    => { job_mtimes[job_idx].insert(key.clone(), current_mtime); let _ = tx.send(SyncMsg::FileSent { name, path: key }); }
                            Ok(SyncResult::Skipped) => { job_mtimes[job_idx].insert(key, current_mtime); let _ = tx.send(SyncMsg::FileSkipped { name }); }
                            Err(e)                  => { let _ = tx.send(SyncMsg::FileError { name, error: e }); }
                        }
    ...
    fn rebuild_sync_jobs(&mut self) {
        let Some(peer) = self.selected_peer().cloned() else { self.sync_jobs.clear(); return; };
        let addr_key = peer.addr.to_string();
        if let Some(folder) = self.sync_map.get(&addr_key).cloned() {
            if folder.exists() {
                self.sync_jobs = vec![SyncJob { folder, peer_addr: peer.addr, peer_name: peer.name.clone(), file_mtimes: std::collections::HashMap::new() }];
            } else { self.sync_jobs.clear(); }
        } else { self.sync_jobs.clear(); }
    }
    ...
    fn selected_peer_available(&self) -> bool { self.selected.is_some() }

    fn poll(&mut self, ctx: &egui::Context) {
        // Process received files / notifications
        if let Ok(mut recv_state) = self.recv_state.try_lock() {
            for file in recv_state.files.iter_mut() {
                if !file.seen && self.notify_on_receive {
                    let _ = notify("File Received", &file.name);
                    file.seen = true;
                    if self.auto_open_folder { open_folder(&file.path); }
                }
            }
        }
        ...
        if let Some(rx) = &self.scan_rx {
            if let Ok(peers) = rx.try_recv() {
                ...
                self.peers = peers.into_iter()
                    .filter(|p| !local_ips.contains(&p.addr.to_string()) && p.name != self.this_hostname)
                    .collect();
                ...
                // Only mark Done once BT scan is also complete
                if self.bt_scan_done {
                    self.scan_state = ScanState::Done;
                } else {
                    // Still waiting for BT; keep state as Scanning but proceed with UI
                    self.scan_state = ScanState::Done; // show WiFi results immediately
                }
        ...
        if let Some(rx) = &self.bt_scan_rx {
            if let Ok(bt_peers) = rx.try_recv() {
                self.bt_scan_rx   = None;
                self.bt_scan_done = true;
                for bp in bt_peers {
                    // Avoid duplicates by BT address
                    let bt_addr = bp.bt_addr.clone().unwrap_or_default();
                    if !self.peers.iter().any(|p| p.bt_addr.as_deref() == Some(bt_addr.as_str())) {
                        self.peers.push(bp);
                    }
                }
            }
            ...
            while let Ok(msg) = rx.try_recv() {
                match msg {
                    QueueMsg::Progress { index, progress } => {
                        if let Some(item) = self.queue.get_mut(index) { item.progress = Some(progress); }
                    }
                    QueueMsg::Done { index } => {
            ...
            while let Ok(msg) = rx.try_recv() {
                match msg {
                    SyncMsg::FileFound { name } => {
                        ...
                        if self.sync_log.len() > 100 { self.sync_log.drain(0..50); }
                    }
        ...
        if let Some(rx) = &self.update_rx {
            if let Ok(v) = rx.try_recv() { self.update_available = v; self.update_rx = None; }
        }

        // Latency pings
        if let Some(rx) = &self.latency_rx {
            while let Ok((addr, ms)) = rx.try_recv() {
                if let Some(p) = self.peers.iter_mut().find(|p| p.addr == addr) { p.latency = Some(ms); }
            }
        }
        ...
        if let Some(rx) = self.relay_rx.take() {
            ...
            while keep && start.elapsed() < std::time::Duration::from_millis(50) {
                match rx.try_recv() {
                    Ok(RelayMsg::Code(code)) => { self.relay_state = RelayState::Online { code }; }
                    Ok(RelayMsg::Paired { peer }) => { self.relay_state = RelayState::Paired { peer_name: peer }; }
                    Ok(RelayMsg::Ready(holder)) => {
                        self.relay_stream  = Some(holder.clone());
                        self.is_relay_mode = true;
                        if let RelayState::Paired { ref peer_name } = self.relay_state {
                            let dummy_ip: std::net::IpAddr = "127.0.0.2".parse().unwrap();
                            let relay_peer = Peer { name: peer_name.clone(), addr: dummy_ip, kind: PeerKind::Remote, latency: None, bt_addr: None };
                            if !self.peers.iter().any(|p| p.addr == dummy_ip) { self.peers.push(relay_peer); }
            ...
            while let Ok(msg) = rx.try_recv() {
                match msg {
                    SyncMsg::FileFound { name } => {
                        ...
                        if self.relay_sync_log.len() > 100 { self.relay_sync_log.drain(0..50); }
                    }
                    SyncMsg::FileSent { name, .. } => {
        ...
        LAST_NETWORK_CHECK.with(|last_check| {
            ...
            if last.elapsed() > std::time::Duration::from_secs(10) {
                if let Some(new_ip) = self.network_monitor.has_changed() {
                    self.this_ip = new_ip;
                    if self.tab == Tab::Scan && self.scan_mode == ScanMode::Local { self.start_scan(); }
                    if matches!(self.relay_state, RelayState::Online { .. } | RelayState::Error(_)) {
                        self.go_offline(); self.go_online();
                    }
                }
        ...
        // Request repaint while a file is actively being received
        let has_incoming = self.recv_state.try_lock().map(|s| s.active_incoming.is_some()).unwrap_or(false);
        if has_incoming {
            ctx.request_repaint_after(std::time::Duration::from_millis(80));
        }
    }

    fn any_active(&self) -> bool { self.queue.iter().any(|q| q.is_active()) }

    fn unread_count(&self) -> usize {
        self.recv_state.lock().unwrap().files.iter().filter(|f| !f.seen).count()
    }
    ...
    fn show_preferences_panel(&mut self, ui: &mut egui::Ui) {
        ...
        egui::Frame::new()
            ...
            .show(ui, |ui| {
                ...
                ui.horizontal(|ui| {
                    ...
                    ui.add_space(8.0);
                    ui.vertical(|ui| {
                        ui.label(RichText::new("Received files folder").size(12.0).color(p.text));
                    ...
                    ui.with_layout(egui::Layout::right_to_left(egui::Align::Center), |ui| {
                        if ui.add(pill_btn("Browse…", p.accent)).clicked() {
                            if let Some(folder) = rfd::FileDialog::new().set_directory(&self.save_dir).pick_folder() {
                                self.save_dir = folder;
                                self.persist_prefs();
                            }
                        }
                        if self.save_dir != dirs::download_dir().unwrap_or_else(|| PathBuf::from(".")) {
                            ui.add_space(6.0);
                            if ui.add(pill_btn("Reset", p.text_dim)).clicked() {
                                self.save_dir = dirs::download_dir().unwrap_or_else(|| PathBuf::from("."));
                                self.persist_prefs();
                            }
                        }
                    });
        ...
        egui::Frame::new()
            ...
            .show(ui, |ui| {
                ...
                ui.horizontal(|ui| {
                    ui.vertical(|ui| {
                        ui.label(RichText::new("Desktop notification on receive").size(12.0).color(p.text));
                        ui.label(RichText::new("Show a system notification when a file arrives").size(10.5).color(p.text_faint));
                    });
                    ui.with_layout(egui::Layout::right_to_left(egui::Align::Center), |ui| {
                ...
                ui.horizontal(|ui| {
                    ui.vertical(|ui| {
                        ui.label(RichText::new("Open folder after receiving").size(12.0).color(p.text));
                        ui.label(RichText::new("Automatically open the save folder when a transfer completes").size(10.5).color(p.text_faint));
                    });
                    ui.with_layout(egui::Layout::right_to_left(egui::Align::Center), |ui| {
                ...
                ui.horizontal(|ui| {
                    ...
                    ui.with_layout(egui::Layout::right_to_left(egui::Align::Center), |ui| {
                        if toggle_switch(ui, &p, self.auto_detect_theme).clicked() {
                            ...
                            if self.auto_detect_theme { self.dark_mode = detect_system_theme(); }
                            self.persist_prefs();
                        }
                    });
                });
    ...
    fn go_online(&mut self) {
        if !matches!(self.relay_state, RelayState::Offline | RelayState::Error(_)) { return; }
        ...
        let tx_clone = tx.clone();
        thread::spawn(move || relay_listen(tx_clone));
                    ...
                    thread::spawn(move || {
                        match receive_file_via_relay(stream, &save_dir) {
                            Ok(f) => {
                                let ts = SystemTime::now().duration_since(UNIX_EPOCH).unwrap_or_default().as_secs();
                                append_history(&HistoryEntry {
                                    timestamp: ts, direction: TransferDir::Received,
                                    transfer_type: TransferType::Remote, file_name: f.name.clone(),
                                    file_size: f.size, peer_name: f.peer_name.clone(),
                                    success: true, error: None, file_path: Some(f.path.clone()),
                                });
                                if let Ok(mut s) = state.lock() {
                                    s.recv_bytes += f.size;
                                    s.recv_files += 1;
                                    if s.auto_open_folder { open_folder(&f.path); }
                                    s.files.push(f);
                                }
                            }
                            Err(e) => {
                                if let Ok(mut s) = state.lock() { s.error = Some(e); }
                            }
                        }
                    });
                }
                            ...
                            if let Ok(mut guard) = stream_arc.lock() {
                                if let Some(mut stream) = guard.take() {
                                    match send_file_via_relay(&path, &name, size, &mut stream, |_| {}) {
                                        Ok(()) => { job_mtimes[job_idx].insert(key.clone(), current_mtime); let _ = tx.send(SyncMsg::FileSent { name, path: key }); }
                                        Err(e) => { let _ = tx.send(SyncMsg::FileError { name, error: e }); }
                                    }
                                    *guard = Some(stream);
                                } else {
                                    let _ = tx.send(SyncMsg::FileError { name: name.clone(), error: "No relay stream available".into() });
                                }
                            }
                        }
                        thread::sleep(std::time::Duration::from_millis(100));
    ...
    fn add_relay_sync_folder(&mut self, folder: PathBuf) {
        ...
        if let Some(peer) = peer {
            let addr_key = peer.addr.to_string();
            if self.relay_sync_active { self.relay_sync_rx = None; self.relay_sync_active = false; }
    ...
    fn rebuild_relay_sync_jobs(&mut self) {
        let Some(peer) = self.selected_peer().cloned() else { self.relay_sync_jobs.clear(); return; };
        let addr_key = peer.addr.to_string();
        if let Some(folder) = self.relay_sync_map.get(&addr_key).cloned() {
            if folder.exists() {
                self.relay_sync_jobs = vec![SyncJob { folder, peer_addr: peer.addr, peer_name: peer.name.clone(), file_mtimes: std::collections::HashMap::new() }];
            } else { self.relay_sync_jobs.clear(); }
        } else { self.relay_sync_jobs.clear(); }
    }
}
...
impl eframe::App for App {
    fn update(&mut self, ctx: &egui::Context, _frame: &mut eframe::Frame) {
        let needs_repaint = self.scan_state == ScanState::Scanning
            ...
            || self.sync_active
            || self.relay_state == RelayState::Connecting
            || self.recv_state.try_lock().map(|s| s.active_incoming.is_some()).unwrap_or(false);
        ...
        let p = self.p();
        ctx.set_visuals({
            let mut v = if self.dark_mode { egui::Visuals::dark() } else { egui::Visuals::light() };
            v.panel_fill         = p.bg;
        ...
        if scanning { self.scan_pulse += ctx.input(|i| i.unstable_dt) * 2.5; }
        ...
        ctx.input(|i| {
            if !i.raw.dropped_files.is_empty() {
                let paths: Vec<PathBuf> = i.raw.dropped_files.iter()
                    .filter_map(|f| f.path.clone()).collect();
                if !paths.is_empty() { self.add_files(paths); self.tab = Tab::Send; }
            }
        });
            ...
            .show(ctx, |ui| {
                ui.horizontal_centered(|ui| {
                    ui.add(egui::Image::new(egui::include_image!("../assets/icon.png"))
                        .fit_to_exact_size(Vec2::splat(28.0)).corner_radius(7.0));
                    ui.add_space(6.0);
                    ...
                    ui.with_layout(egui::Layout::right_to_left(egui::Align::Center), |ui| {
                        ...
                        for (lbl, t, locked) in tab_defs {
                                ...
                                if *locked { self.show_upgrade = true; } else {
                                    ...
                                    if self.tab == Tab::History {
                                        if let Ok(mut rs) = self.recv_state.lock() {
                                            for f in &mut rs.files { f.seen = true; }
                                        }
                            ...
                            if *lbl == "History" && history_new > 0 {
                                ...
                                ui.painter().text(dot, egui::Align2::CENTER_CENTER,
                                    if history_new > 9 { "9+".to_string() } else { history_new.to_string() },
                                    egui::FontId::proportional(7.5), p.text);
                            }
                            ui.add_space(2.0);
                        }
                    });
        ...
        egui::TopBottomPanel::bottom("statusbar")
            ...
            .show(ctx, |ui| {
                ui.horizontal_centered(|ui| {
                    ui.label(RichText::new(format!("{} {}", env!("CARGO_PKG_NAME"), self.version))
                        .size(10.5).color(p.text_faint));
                    ...
                    if let Some(ref latest) = self.update_available {
                        ...
                        let resp = ui.add(egui::Button::new(
                            RichText::new(format!("{} {} available", icons::ICON_ARROW_UPWARD, latest))
                                .size(10.0).color(p.warn).strong()).frame(false));
                        if resp.clicked() {
                            open_url(format!("{}/releases/latest", env!("CARGO_PKG_REPOSITORY")).as_str());
                        }
                    }

                    // ── Live incoming progress widget ──────────────────────
                    // Read active_incoming without blocking
                    let incoming_snapshot = self.recv_state.try_lock().ok()
                        .and_then(|s| s.active_incoming.clone());

                    if let Some(ref inc) = incoming_snapshot {
                        let progress = if inc.file_size > 0 {
                            (inc.received_bytes as f32 / inc.file_size as f32).clamp(0.0, 1.0)
                        } else { 0.0 };

                        // Show a compact progress pill in the centre of the status bar
                        ui.with_layout(egui::Layout::centered_and_justified(egui::Direction::LeftToRight), |ui| {
                            ui.horizontal(|ui| {
                                ui.spinner();
                                ui.add_space(4.0);

                                let batch_label = if inc.batch_total_files > 1 {
                                    format!("file {}/{}", inc.batch_file_index, inc.batch_total_files)
                                } else {
                                    "file".to_string()
                                };
                                ...
                                let recv_label = format!(
                                    "Receiving {} ({}) from {} — {} / {}",
                                    truncate_filename(&inc.file_name, 22),
                                    batch_label,
                                    &inc.peer_name,
                                    format_size(inc.received_bytes),
                                    format_size(inc.file_size),
                                );
                                ui.label(RichText::new(&recv_label).size(10.0).color(p.text_dim));
                                ui.add_space(6.0);
                        ...
                        ui.with_layout(egui::Layout::centered_and_justified(egui::Direction::LeftToRight), |ui| {
                            ui.horizontal(|ui| {
                                let total_sent = self.history.iter().filter(|e| e.direction == TransferDir::Sent    && e.success).count();
                                let total_recv = self.history.iter().filter(|e| e.direction == TransferDir::Received && e.success).count();
                                let bytes_sent: u64 = self.history.iter().filter(|e| e.direction == TransferDir::Sent    && e.success).map(|e| e.file_size).sum();
                                let bytes_recv: u64 = self.history.iter().filter(|e| e.direction == TransferDir::Received && e.success).map(|e| e.file_size).sum();
                                status_metric(ui, &p, icons::ICON_UPLOAD,
                                    &format!("{} · {} file{}", format_size(bytes_sent), total_sent, if total_sent == 1 { "" } else { "s" }));
                                ui.add_space(16.0);
...
impl App {
                    ...
                    ui.horizontal(|ui| {
                        ...
                        if self.is_pro() {
                            for (label, mode) in [("  Local  ", ScanMode::Local), ("  Remote  ", ScanMode::Remote)] {
                                ...
                                if ui.add(egui::Button::new(RichText::new(label).size(12.0).color(text_col))
                                    .fill(fill).corner_radius(6.0).min_size(Vec2::new(0.0, 30.0))).clicked()
                                {
                                    self.scan_mode = mode;
                                }
                            }
                            ...
                            if n > 0 {
                                ...
                                egui::Frame::new()
                                    ...
                                    .show(ui, |ui| {
                                        ...
                                        ui.horizontal(|ui| {
                                            ui.label(RichText::new(icons::ICON_SEARCH).size(13.0).color(p.text_faint));
                                            ui.add_space(4.0);
                                            ui.add(egui::TextEdit::singleline(&mut self.scan_filter)
                                                .hint_text("Filter…").desired_width(110.0).frame(false));
                                            if !self.scan_filter.is_empty() {
                                                if ui.add(egui::Button::new(RichText::new(icons::ICON_CLOSE).size(11.0).color(p.text_faint)).frame(false)).clicked() {
                                ...
                                if self.scan_state == ScanState::Done {
                                    ui.add_space(8.0);
                                    ui.label(RichText::new(format!("{} device{}", n, if n == 1 { "" } else { "s" }))
                            ...
                            ui.with_layout(egui::Layout::right_to_left(egui::Align::Center), |ui| {
                                match &self.relay_state {
                                    RelayState::Online { .. } | RelayState::Paired { .. } => {
                                        if ui.add(pill_btn("Go Offline", p.danger)).clicked() { self.go_offline(); }
                                        ui.add_space(8.0);
                                        ui.painter().circle_filled(ui.cursor().left_top() + Vec2::new(4.0, 10.0), 5.0, p.success);
                                        ui.add_space(14.0);
                                        ui.label(RichText::new("Online").size(11.5).color(p.success).strong());
                                    }
                                    RelayState::Connecting => {
                                        ui.spinner();
                                        ui.add_space(6.0);
                                        ui.label(RichText::new("Connecting…").size(11.5).color(p.text_dim));
                                    }
                                    _ => {
                                        if ui.add(egui::Button::new(RichText::new("  Go Online  ").size(12.0).color(Color32::WHITE))
                                            .fill(p.success).corner_radius(6.0).min_size(Vec2::new(0.0, 30.0))).clicked()
                                        {
                                            self.go_online();
                                        }
                                        ui.add_space(8.0);
                                        ui.painter().circle_filled(ui.cursor().left_top() + Vec2::new(4.0, 10.0), 5.0, p.text_faint);
                                        ui.add_space(14.0);
                                        ui.label(RichText::new("Offline").size(11.5).color(p.text_faint));
                                    }
                                }
                            });
                        }
    ...
    fn show_local_scan(&mut self, ui: &mut egui::Ui, ctx: &egui::Context) {
        ...
        if self.scan_state == ScanState::Idle {
            ...
            ui.vertical_centered(|ui| {
                ...
                ui.add_space(16.0);
                ui.label(RichText::new("Find devices on your network").size(14.0).strong().color(p.text));
                ui.add_space(6.0);
                ui.label(RichText::new(format!("Devices running {} on the same Wi-Fi or via Bluetooth will appear here.", env!("CARGO_PKG_NAME")))
                    .size(12.0).color(p.text_dim));
                ui.add_space(20.0);
                if ui.add(big_btn(&format!("  {}  Scan  ", icons::ICON_SEARCH), p.accent)).clicked() { self.start_scan(); }
            });
            ui.add_space(32.0);
            return;
        }
        ...
        if scanning {
            ...
            ui.vertical_centered(|ui| {
                ...
                ui.add_space(14.0);
                ui.label(RichText::new("Scanning network + Bluetooth…").size(13.0).color(p.text_dim));
            });
            ui.add_space(32.0);
            ctx.request_repaint_after(std::time::Duration::from_millis(40));
            return;
        ...
        if self.scan_state == ScanState::Done {
            ...
            let local_peers: Vec<(usize, Peer)> = {
                let mut tmp: Vec<_> = all_peers.iter().enumerate()
            ...
            let filtered_local: Vec<(usize, Peer)> = local_peers.into_iter()
            ...
            // ── Bluetooth peers ───────────────────────────────────────────
            let bt_peers: Vec<(usize, Peer)> = all_peers.iter().enumerate()
                .filter(|(_, p)| matches!(p.kind, PeerKind::Bluetooth(_)))
                .filter(|(_, p)| filter.is_empty()
                    || p.name.to_lowercase().contains(&filter)
                    || p.bt_addr.as_deref().unwrap_or("").to_lowercase().contains(&filter))
                .map(|(i, p)| (i, p.clone()))
                .collect();

            // ── Remote (manual) peers ─────────────────────────────────────
            let remote_peers: Vec<(usize, Peer)> = all_peers.iter().enumerate()
                .filter(|(_, p)| p.kind == PeerKind::Remote)
                .filter(|(_, p)| filter.is_empty()
                    || p.name.to_lowercase().contains(&filter)
                    || p.addr.to_string().contains(&filter))
                .map(|(i, p)| (i, p.clone()))
            ...
            let has_any = !filtered_local.is_empty() || !bt_peers.is_empty() || !remote_peers.is_empty();
            ...
            if has_any {
                ...
                // WiFi peers
                for (peer_idx, peer) in &filtered_local {
                    let is_recent = peer.addr.to_string() == self.saved_peer_addr;
                    let sel  = self.selected == Some(*peer_idx);
                    let peer_ref = peer.clone();
                    let resp = device_row(ui, &p, &peer_ref, sel, is_recent, false);
                    if resp.clicked()        { self.select_peer(*peer_idx, &peer_ref); }
                    if resp.double_clicked() { self.select_peer(*peer_idx, &peer_ref); self.tab = Tab::Send; }
                }

                // Bluetooth peers section
                if !bt_peers.is_empty() {
                    ui.add_space(8.0);
                    egui::Frame::new()
                        .fill(tint(p.warn, 8))
                        .inner_margin(egui::Margin { left: 16, right: 16, top: 4, bottom: 4 })
                        .show(ui, |ui| {
                            ui.set_min_width(ui.available_width());
                            ui.horizontal(|ui| {
                                ui.label(RichText::new("🔵  Bluetooth devices").size(10.5).strong().color(p.warn));
                                ui.add_space(8.0);
                                ui.label(RichText::new("(paired)").size(9.5).color(p.text_faint));
                            });
                        });
                    for (peer_idx, peer) in &bt_peers {
                        let sel      = self.selected == Some(*peer_idx);
                        let peer_ref = peer.clone();
                        let resp     = device_row(ui, &p, &peer_ref, sel, false, false);
                        if resp.clicked()        { self.select_peer(*peer_idx, &peer_ref); }
                        if resp.double_clicked() { self.select_peer(*peer_idx, &peer_ref); self.tab = Tab::Send; }
                    }
                }

                // Manual / remote peers section
                if !remote_peers.is_empty() {
                    ui.add_space(8.0);
                    egui::Frame::new()
                        .fill(tint(p.accent2, 8))
                        .inner_margin(egui::Margin { left: 16, right: 16, top: 4, bottom: 4 })
                        .show(ui, |ui| {
                            ui.set_min_width(ui.available_width());
                            ui.label(RichText::new("🌐  Remote / manual peers").size(10.5).strong().color(p.accent2));
                        });
                    for (peer_idx, peer) in &remote_peers {
                        let is_recent = peer.addr.to_string() == self.saved_peer_addr;
                        let sel      = self.selected == Some(*peer_idx);
                        let peer_ref = peer.clone();
                        let resp     = device_row(ui, &p, &peer_ref, sel, is_recent, true);
                        if resp.clicked()        { self.select_peer(*peer_idx, &peer_ref); }
                        if resp.double_clicked() { self.select_peer(*peer_idx, &peer_ref); self.tab = Tab::Send; }
                    }
                }
                ...
                ui.vertical_centered(|ui| {
                    ...
                    ui.add_space(8.0);
                    ui.label(RichText::new("No devices found").size(14.0).strong().color(p.text));
                    ui.add_space(4.0);
                    ui.label(RichText::new(format!(
                        "Make sure the other device is on the same Wi-Fi and running {}, or pair via Bluetooth.",
                        env!("CARGO_PKG_NAME"))).size(11.5).color(p.text_dim));
                    ui.add_space(12.0);
                    if ui.add(pill_btn("Scan again", p.accent)).clicked() { self.start_scan(); }
                });
            }
        }
    }
                ...
                match self.relay_state.clone() {
                    RelayState::Offline | RelayState::Error(_) => {
                            ...
                            egui::Frame::new()
                                ...
                                .show(ui, |ui| {
                                    ...
                                    ui.horizontal(|ui| {
                                        ...
                                        ui.add_space(8.0);
                                        if ui.add(pill_btn("Retry", p.accent)).clicked() { self.go_online(); }
                                    });
                                });
                            ui.add_space(12.0);
                        }
                        ...
                        if self.is_online {
                                    ...
                                    egui::Frame::new()
                                        ...
                                        .show(ui, |ui| {
                                            ...
                                            ui.vertical_centered(|ui| {
                                                ...
                                                ui.add_space(6.0);
                                                ui.label(RichText::new("Receive files").size(13.0).strong().color(p.text));
                                                ui.add_space(4.0);
                                                ui.label(RichText::new("Go online to get a share code").size(11.0).color(p.text_dim));
                                                ui.add_space(10.0);
                                                if ui.add(egui::Button::new(RichText::new("  Go Online  ").size(12.5).color(Color32::WHITE))
                                    ...
                                    egui::Frame::new()
                                        ...
                                        .show(ui, |ui| {
                                            ...
                                            ui.vertical_centered(|ui| {
                                                ui.label(RichText::new("📤").size(28.0));
                                                ui.add_space(6.0);
                                                ui.label(RichText::new("Send files").size(13.0).strong().color(p.text));
                                                ui.add_space(4.0);
                                                ...
                                                egui::Frame::new()
                                                    ...
                                                    .show(ui, |ui| {
                                                        ui.set_width(ui.available_width());
                                                        ui.add(egui::TextEdit::singleline(&mut self.relay_code_input)
                                                            .hint_text("XXXX-XXXX").desired_width(f32::INFINITY).frame(false));
                                                    });
                                                ui.add_space(6.0);
                                                let can = self.relay_code_input.trim().len() == 9;
                                                ui.add_enabled_ui(can, |ui| {
                                                    if ui.add(egui::Button::new(RichText::new("  Connect  ").size(12.5).color(Color32::WHITE))
                            ...
                            ui.vertical_centered(|ui| {
                                ui.label(RichText::new("📡").size(40.0));
                                ui.add_space(12.0);
                                ui.label(RichText::new("No Internet Connection").size(16.0).strong().color(p.text));
                                ui.add_space(8.0);
                                ui.label(RichText::new("Please check your network connection and try again.").size(12.0).color(p.text_dim));
                                ui.add_space(16.0);
                        ...
                        egui::Frame::new()
                            ...
                            .show(ui, |ui| {
                                ...
                                ui.vertical(|ui| {
                                    ...
                                    ui.add_space(4.0);
                                    for (n, t) in [
                                        ("1.", "Receiver clicks 'Go Online' to get a share code"),
                                        ("2.", "Receiver shares the code with the sender"),
                                        ("3.", "Sender enters the code and connects"),
                                        ("4.", "Files transfer securely via relay"),
                                    ] {
                                        ui.horizontal(|ui| {
                                            ui.label(RichText::new(n).size(10.0).color(p.text_dim));
                                            ui.label(RichText::new(t).size(10.0).color(p.text_faint));
                                        });
                                    }
                                });
                            });
                    ...
                    RelayState::Connecting => {
                        ...
                        ui.vertical_centered(|ui| {
                            ui.spinner();
                            ui.add_space(12.0);
                            ui.label(RichText::new("Connecting to relay…").size(13.0).color(p.text_dim));
                            ui.add_space(6.0);
                            ui.label(RichText::new("This only takes a moment").size(11.0).color(p.text_faint));
                            ui.add_space(16.0);
                            if ui.add(pill_btn("Cancel", p.danger)).clicked() { self.go_offline(); }
                        });
                    }
                    ...
                    RelayState::Online { ref code } => {
                        ...
                        ui.vertical_centered(|ui| {
                            ui.horizontal(|ui| {
                                ...
                                ui.add_space(6.0);
                                ui.label(RichText::new("You're online — waiting for sender…").size(12.0).color(p.success));
                            });
                            ui.add_space(20.0);
                            ...
                            egui::Frame::new()
                                ...
                                .show(ui, |ui| {
                                    ui.label(RichText::new(&code).size(32.0).strong().color(p.accent).monospace());
                                });
                            ui.add_space(12.0);
                            if ui.add(pill_btn("Copy code", p.accent)).clicked() { ui.ctx().copy_text(code.clone()); }
                            ui.add_space(6.0);
                            ui.label(RichText::new("Share this code with the sender.").size(11.0).color(p.text_dim));
                            ui.label(RichText::new("Code expires when used or after 5 minutes.").size(10.5).color(p.text_faint));
                            ui.add_space(16.0);
                            if ui.add(pill_btn("Cancel", p.danger)).clicked() { self.go_offline(); }
                        });
                    }
                    ...
                    RelayState::Paired { ref peer_name } => {
                        ...
                        ui.vertical_centered(|ui| {
                            ...
                            ui.add_space(8.0);
                            ui.label(RichText::new(format!("Connected to {}", peer_name)).size(14.0).strong().color(p.text));
                            ui.add_space(4.0);
                            ui.label(RichText::new("Switching to Send tab…").size(11.0).color(p.text_dim));
                        });
                    }
                }
            });
    ...
    fn show_send(&mut self, ui: &mut egui::Ui, ctx: &egui::Context, wide: bool) {
        ...
        if let Some(peer) = self.selected_peer().cloned() {
            ...
            egui::Frame::new()
                ...
                .show(ui, |ui| {
                    ui.set_min_width(ui.available_width());
                    ui.horizontal(|ui| {
                        let dot_col = match &peer.kind { PeerKind::Bluetooth(_) => p2.warn, _ => p2.success };
                        ui.painter().circle_filled(ui.cursor().left_top() + Vec2::new(4.0, 8.0), 4.0, dot_col);
                        ui.add_space(12.0);
                        let addr_display = match &peer.kind {
                            PeerKind::Bluetooth(bt) => format!("BT: {}", bt),
                            _ => peer.addr.to_string(),
                        };
                        ui.label(RichText::new(format!("Selected  {}  ·  {}", peer.name, addr_display))
                            .size(12.0).color(p2.accent));
                        ui.with_layout(egui::Layout::right_to_left(egui::Align::Center), |ui| {
                            if ui.add(egui::Button::new(RichText::new("Change").size(11.0).color(p2.text_dim)).frame(false)).clicked() {
            ...
            egui::Frame::new()
                ...
                .show(ui, |ui| {
                    ui.set_min_width(ui.available_width());
                    ui.horizontal(|ui| {
                        ui.label(RichText::new("⚠  No device selected").size(12.0).color(p2.warn));
                        ui.with_layout(egui::Layout::right_to_left(egui::Align::Center), |ui| {
                            if ui.add(egui::Button::new(RichText::new("Scan now").size(11.0).color(p2.accent)).frame(false)).clicked() {
                                self.scan_mode = ScanMode::Local;
                                self.tab       = Tab::Scan;
                                self.start_scan();
                            }
        ...
        if wide {
            ...
            ui.allocate_ui_with_layout(
                ...
                |ui| {
                    ui.add_space(pad);
                    ui.vertical(|ui| { ui.set_width(col); self.card_queue(ui, ctx); });
                    ui.add_space(gap);
                    ui.vertical(|ui| { ui.set_width(col); self.card_send_action(ui, ctx); });
                },
            );
        } else {
            egui::ScrollArea::vertical().auto_shrink([false, false]).show(ui, |ui| {
    ...
    fn card_queue(&mut self, ui: &mut egui::Ui, ctx: &egui::Context) {
        ...
        card(ui, &p, |ui| {
            ui.horizontal(|ui| {
                ...
                ui.add_space(8.0);
                let hdr = if has_items { format!("Files  ({})", self.queue.len()) } else { "Files to Send".to_string() };
                ui.label(RichText::new(hdr).strong().size(14.0).color(p.text));
                ui.with_layout(egui::Layout::right_to_left(egui::Align::Center), |ui| {
                    if ui.add(pill_btn(if has_items { "Add files…" } else { "Browse…" }, p.accent)).clicked() {
                        if let Some(paths) = rfd::FileDialog::new().pick_files() { self.add_files(paths); }
                    }
                    if done_count > 0 {
                        ui.add_space(4.0);
                        if ui.add(pill_btn("Clear done", p.text_dim)).clicked() { self.clear_done(); }
                    }
                    if failed_count > 0 {
                        ui.add_space(4.0);
                        if ui.add(pill_btn("Retry failed", p.warn)).clicked() { self.retry_failed(); }
                    }
                });
            });
            ui.add_space(10.0);
            ...
            if has_items {
                ...
                if let Some(idx) = remove_idx { self.remove_queue_item(idx); }
                ui.add_space(4.0);
        ...
        card(ui, &p, |ui| {
            if active {
                ...
                ui.horizontal(|ui| {
                    ...
                    ui.label(RichText::new(format!("Sending {} file{}…", sending, if sending == 1 { "" } else { "s" }))
                        .size(13.0).color(p.text));
                });
                ui.add_space(8.0);
                let overall = if total > 0 {
                    self.queue.iter().map(|q| q.progress.unwrap_or(0.0).max(0.0)).sum::<f32>() / total as f32
                } else { 0.0 };
                ...
                if done + failed == total {
                    ...
                    let msg = if failed > 0 {
                        ...
                        format!("{} All {} file{} sent!", icons::ICON_CHECK, total, if total == 1 { "" } else { "s" })
                    };
                    ui.label(RichText::new(msg).size(12.0).color(if failed > 0 { p.warn } else { p.success }));
                }
            } else {
                ui.horizontal(|ui| { check_item(ui, &p, !self.queue.is_empty(), "Files added"); });
                ui.add_space(12.0);
                let btn_label = if pending_count > 0 {
                    format!("  {}  Send {} file{}  ", icons::ICON_SEND, pending_count, if pending_count == 1 { "" } else { "s" })
                } else {
                ...
                ui.add_enabled_ui(can_send, |ui| {
                    if ui.add(egui::Button::new(RichText::new(&btn_label).size(14.0).strong().color(btn_txt))
                        .fill(btn_fill).corner_radius(12.0).min_size(Vec2::new(ui.available_width(), 48.0))).clicked()
                    {
    ...
    fn show_history(&mut self, ui: &mut egui::Ui, _ctx: &egui::Context, wide: bool) {
        ...
        let col_w  = if wide { (avail_w - 64.0).min(700.0) } else { avail_w - 32.0 };
                ...
                ui.vertical(|ui| {
                    ...
                    ui.horizontal(|ui| {
                        if !self.history.is_empty() {
                            ...
                            ui.add_space(8.0);
                            ui.label(RichText::new("Transfer History").strong().size(15.0).color(p.text));
                        }
                        ui.with_layout(egui::Layout::right_to_left(egui::Align::Center), |ui| {
                            if !self.history.is_empty() {
                                if ui.add(pill_btn("Clear all", p.danger)).clicked() {
                                    if let Some(path) = history_path() { let _ = fs::remove_file(&path); }
                                    self.history.clear();
                                    self.history_filter.clear();
                                }
                                ui.add_space(6.0);
                                let count = self.history.len();
                                ui.label(RichText::new(format!("{} transfer{}", count, if count == 1 { "" } else { "s" }))
                                    .size(11.0).color(p.text_faint));
                            }
                        });
                    });
                        ...
                        egui::Frame::new()
                            ...
                            .show(ui, |ui| {
                                ...
                                ui.horizontal(|ui| {
                                    ...
                                    ui.add_space(3.0);
                                    let resp = ui.add(egui::TextEdit::singleline(&mut self.history_filter)
                                        .hint_text("Search by filename or device…")
                                        .desired_width(ui.available_width() - 33.0).frame(false));
                    ...
                    let entries: Vec<HistoryEntry> = self.history.iter()
                    ...
                    if entries.is_empty() {
                        ui.vertical_centered(|ui| {
                            ...
                            ui.add_space(14.0);
                            ui.label(RichText::new(if filter.is_empty() { "No transfers yet" } else { "No results" })
                                .strong().size(15.0).color(p.text));
                            ui.add_space(6.0);
                            ui.label(RichText::new("File transfers appear here when you send or receive files")
                                .size(12.0).color(p.text_dim));
                            ui.add_space(4.0);
                            if let Some(dir) = dirs::download_dir() {
                                ui.label(RichText::new(format!("All files saved to {}", dir.display()))
                                    .size(11.0).color(p.text_faint));
                            }
                        });
                    } else {
                        for entry in &entries { history_row(ui, &p, entry); ui.add_space(6.0); }
                    }
                });
            });
            ui.add_space(24.0);
    ...
    fn show_sync(&mut self, ui: &mut egui::Ui, ctx: &egui::Context, wide: bool) {
        ...
        let sync_log_clone = if is_relay_mode { self.relay_sync_log.clone() } else { self.sync_log.clone() };
        let sync_active    = if is_relay_mode { self.relay_sync_active } else { self.sync_active };
        let current_folder = if is_relay_mode {
            self.selected_peer().and_then(|peer| self.relay_sync_map.get(&peer.addr.to_string())).cloned()
        } else {
            self.selected_peer().and_then(|peer| self.sync_map.get(&peer.addr.to_string())).cloned()
        };
        ...
        let has_folder = if is_relay_mode {
            self.selected_peer().and_then(|p| self.relay_sync_map.get(&p.addr.to_string())).is_some()
        } else {
            self.selected_peer().and_then(|p| self.sync_map.get(&p.addr.to_string())).is_some()
        };
            ...
            ui.horizontal(|ui| {
                ...
                ui.vertical(|ui| {
                    ...
                    ui.horizontal(|ui| {
                        icon_badge(ui, "📁", p.accent);
                        ui.add_space(8.0);
                        ui.label(RichText::new(if is_relay_mode { "Remote Folder Sync" } else { "Folder Sync" })
                            .strong().size(15.0).color(p.text));
                        ui.with_layout(egui::Layout::right_to_left(egui::Align::Center), |ui| {
                            if is_relay_mode {
                                if sync_active {
                                    if ui.add(pill_btn(&format!("{}  Stop remote sync", icons::ICON_STOP_CIRCLE), p.danger)).clicked() { self.stop_relay_sync(); }
                                    ui.add_space(8.0); ui.spinner(); ui.add_space(4.0);
                                    ui.label(RichText::new("Syncing remotely…").size(11.0).color(p.success));
                                } else if peer_available && has_folder {
                                    if ui.add(big_btn(&format!("{}  Start remote sync", icons::ICON_SYNC), p.accent)).clicked() {
                                        self.rebuild_relay_sync_jobs(); self.start_relay_sync_watcher();
                                    }
                                }
                            } else {
                                if sync_active {
                                    if ui.add(pill_btn(&format!("{}  Stop watching", icons::ICON_STOP_CIRCLE), p.danger)).clicked() { self.stop_sync(); }
                                    ui.add_space(8.0); ui.spinner(); ui.add_space(4.0);
                                    ...
                                    if ui.add(big_btn(&format!("{}  Start watching", icons::ICON_SYNC), p.accent)).clicked() {
                                        self.rebuild_sync_jobs(); self.start_sync_watcher();
                                    }
                                }
                            }
                        });
                    ...
                    ui.add_space(6.0);
                    ui.label(RichText::new(if is_relay_mode {
                        "Add folders below. New files added will be automatically sent to the remote device via relay."
                    } else {
                        "Add folders below. New files dropped in are automatically sent to the selected device."
                    }).size(12.0).color(p.text_dim));
                    ui.add_space(14.0);
                        ...
                        egui::Frame::new().fill(tint(p.warn, 12)).stroke(Stroke::new(1.0_f32, tint(p.warn, 50)))
                            ...
                            .show(ui, |ui| {
                                ui.horizontal(|ui| {
                                    ...
                                    ui.add_space(6.0);
                                    ui.label(RichText::new(if is_relay_mode {
                                        "No remote connection active. Connect via remote mode first.".to_string()
                                    } else if !self.saved_peer_name.is_empty() {
                                        format!("{} is not available  ·  scan to reconnect", self.saved_peer_name)
                                    } else {
                                        "Select a device on the Send tab first".to_string()
                    ...
                    if let Some(ref folder) = current_folder {
                        ...
                        egui::Frame::new().fill(fill_col).stroke(Stroke::new(1.0_f32, border_col))
                            ...
                            .show(ui, |ui| {
                                ...
                                ui.horizontal(|ui| {
                                    ...
                                    ui.vertical(|ui| {
                                        ui.horizontal(|ui| {
                                            ui.label(RichText::new(folder.file_name().unwrap_or_default().to_string_lossy())
                                                .strong().size(13.0).color(p.text));
                                            ui.add_space(6.0);
                                            if sync_active {
                                                status_badge(ui, if is_relay_mode { "REMOTE SYNC" } else { "WATCHING" }, p.success);
                                            }
                                            if !folder_exists { status_badge(ui, "MISSING", p.warn); }
                                        });
                                        let full = folder.to_string_lossy();
                                        let display = if full.len() > 50 { format!("…{}", &full[full.len().saturating_sub(48)..]) } else { full.to_string() };
                                        ui.label(RichText::new(format!("{}  ·  {} file{} synced{}",
                                            display, sent_count, if sent_count == 1 { "" } else { "s" },
                                            if is_relay_mode { " remotely" } else { "" }))
                                            .size(11.0).color(p.text_dim));
                                    });
                                    ui.with_layout(egui::Layout::right_to_left(egui::Align::Center), |ui| {
                                        if ui.add(egui::Button::new(RichText::new(icons::ICON_CLOSE).size(13.0).color(p.text_dim)).frame(false)).clicked() {
                                            if is_relay_mode { self.remove_relay_sync_folder(); } else { self.remove_sync_folder(); }
                                        }
                                    });
                                });
                            });
                        ...
                        ui.vertical_centered(|ui| {
                            ...
                            ui.add_space(10.0);
                            ui.label(RichText::new("No folder selected yet").size(13.0).color(p.text_dim));
                            ui.add_space(4.0);
                            ui.label(RichText::new(if is_relay_mode {
                    ...
                    if !sync_log_clone.is_empty() && peer_available {
                        ...
                        ui.horizontal(|ui| {
                            ui.label(RichText::new("Activity log").size(12.0).strong().color(p.text_dim));
                            ui.with_layout(egui::Layout::right_to_left(egui::Align::Center), |ui| {
                                if ui.add(pill_btn("Clear", p.text_faint)).clicked() {
                                    if is_relay_mode { self.relay_sync_log.clear(); } else { self.sync_log.clear(); }
                                }
                            });
                        });
                        ui.add_space(6.0);
                        let remaining = ui.available_height().max(180.0);
                        egui::Frame::new().fill(p.surface2).corner_radius(8.0).inner_margin(egui::Margin::same(10))
                            .show(ui, |ui| {
                                ui.set_min_size(Vec2::new(ui.available_width(), remaining - 20.0));
                                egui::ScrollArea::vertical().max_height(f32::INFINITY).auto_shrink([false, false]).stick_to_bottom(true)
                                    .show(ui, |ui| {
                                        ui.set_min_width(ui.available_width());
                                        for line in sync_log_clone.iter().rev() {
                                            let (col, prefix) = if line.starts_with("ERR") { (p.danger, "") }
                                                else if line.starts_with("OK") { (p.success, "") }
                                                else { (p.text_dim, "") };
                                            ui.label(RichText::new(format!("{}{}", prefix, line)).size(10.5).color(col).monospace());
                                        }
                                    });
                            });
                    }
    ...
    fn show_settings(&mut self, ui: &mut egui::Ui, _ctx: &egui::Context, wide: bool) {
        let p      = self.p();
        let avail_w = ui.available_width();
        let col_w  = if wide { (avail_w - 64.0).min(600.0) } else { avail_w - 32.0 };
        let x_pad  = (avail_w - col_w) / 2.0;
        egui::ScrollArea::vertical().auto_shrink([false, false]).show(ui, |ui| {
            ui.add_space(24.0);
            ui.horizontal(|ui| {
                ...
                ui.vertical(|ui| {
                    ...
                    ui.horizontal(|ui| {
                        for (lbl, st) in [
                            ...
                            let fill = if active { tint(p.accent, 20) } else { Color32::TRANSPARENT };
                            if ui.add(egui::Button::new(RichText::new(lbl).size(12.0).color(col))
                                .fill(fill).corner_radius(6.0).min_size(Vec2::new(0.0, 32.0))).clicked()
                            {
    ...
    fn show_device_panel(&self, ui: &mut egui::Ui) {
        ...
        if !self.saved_peer_name.is_empty() {
            egui::Frame::new()
                ...
                .show(ui, |ui| {
                    ...
                    ui.horizontal(|ui| {
                        let (r, _) = ui.allocate_exact_size(Vec2::splat(38.0), Sense::hover());
                        ui.painter().circle_filled(r.center(), 18.0, tint(p.accent, 25));
                        ui.painter().text(r.center(), egui::Align2::CENTER_CENTER,
                            &self.saved_peer_name.chars().next().unwrap_or('?').to_uppercase().to_string(),
                            egui::FontId::proportional(18.0), p.accent);
                        ui.add_space(12.0);
                        ui.vertical(|ui| {
                            ui.label(RichText::new("Last connected peer").size(11.0).color(p.text_faint));
                            ui.label(RichText::new(&self.saved_peer_name).strong().size(14.0).color(p.text));
                            ui.label(RichText::new(&self.saved_peer_addr).size(11.0).color(p.text_dim));
                        });
                    });
                });
            ui.add_space(16.0);
        ...
        egui::Frame::new()
            ...
            .show(ui, |ui| {
                ui.set_min_width(ui.available_width());
                ui.label(RichText::new("This Device").strong().size(13.0).color(p.text_dim));
                ui.add_space(10.0);
                info_row(ui, &p, icons::ICON_COMPUTER, "Hostname", &self.this_hostname);
                ui.add_space(6.0);
                info_row(ui, &p, icons::ICON_WIFI, "IP Address", &self.this_ip);
                ui.add_space(6.0);
                info_row(ui, &p, icons::ICON_LOCK, "Transfer port", &TRANSFER_PORT.to_string());
            });
        ...
        if self.session_sent_bytes > 0 || self.session_sent_files > 0 {
            ...
            egui::Frame::new()
                ...
                .show(ui, |ui| {
                    ui.set_min_width(ui.available_width());
                    ui.label(RichText::new("This Session").strong().size(13.0).color(p.text_dim));
                    ui.add_space(10.0);
                    info_row(ui, &p, icons::ICON_UPLOAD, "Files sent",  &self.session_sent_files.to_string());
                    ui.add_space(6.0);
                    info_row(ui, &p, icons::ICON_UPLOAD, "Data sent",   &format_size(self.session_sent_bytes));
                });
        } else {
            ui.add_space(24.0);
            ui.vertical_centered(|ui| {
                ui.label(RichText::new("No device selected yet").size(12.0).color(p.text_faint));
                ui.add_space(4.0);
    ...
    fn show_about_panel(&self, ui: &mut egui::Ui) {
        ...
        ui.vertical_centered(|ui| {
            ui.add_space(16.0);
            ui.add(egui::Image::new(egui::include_image!("../assets/icon.png"))
                .fit_to_exact_size(Vec2::splat(64.0)).corner_radius(14.0));
            ui.add_space(10.0);
            ui.label(RichText::new(env!("CARGO_PKG_NAME")).strong().size(18.0).color(p.text));
            ui.label(RichText::new(self.version.clone()).size(12.0).color(p.text_dim));
            ui.add_space(6.0);
            ui.label(RichText::new(env!("CARGO_PKG_DESCRIPTION")).size(12.0).color(p.text_dim));
            ui.add_space(16.0);
            for (label, url) in [
                ...
                if ui.add(pill_btn(label, p.accent)).clicked() { open_url(url.as_str()); }
                ui.add_space(6.0);
            }
        });
    }
    ...
    fn check_for_update(&mut self) {
        ...
        thread::spawn(move || {
            ...
            if let Some(latest) = latest {
                if is_newer(&latest, &current) { let _ = tx.send(Some(latest)); return; }
            }
            let _ = tx.send(None);
        });
    }
...
#[cfg(target_os = "linux")]
async fn write_encrypted_async<W: tokio::io::AsyncWrite + Unpin>(
    stream: &mut W,
    cipher: &Aes256Gcm,
    pt: &[u8],
) -> Result<(), String> {
    ...
    stream.write_all(&(enc.len() as u32).to_le_bytes()).await.map_err(|e| e.to_string())?;
    stream.write_all(&enc).await.map_err(|e| e.to_string())?;
    Ok(())
}

/// Async decrypt-after-read helper for Bluetooth streams.
#[cfg(target_os = "linux")]
async fn read_encrypted_async<R: tokio::io::AsyncRead + Unpin>(
    stream: &mut R,
    cipher: &Aes256Gcm,
) -> Result<Vec<u8>, String> {
    use tokio::io::AsyncReadExt;
    let mut lb = [0u8; 4];
    stream.read_exact(&mut lb).await.map_err(|e| e.to_string())?;
    let len = u32::from_le_bytes(lb) as usize;
    if len > CHUNK_SIZE + AES_TAG_LEN + AES_NONCE_LEN + 64 {
        return Err(format!("BT packet too large: {}", len));
    }
    let mut enc = vec![0u8; len];
    stream.read_exact(&mut enc).await.map_err(|e| e.to_string())?;
...
fn receive_server(state: Arc<Mutex<RecvState>>, save_dir: PathBuf) {
    ...
    let listener = match TcpListener::bind(("0.0.0.0", TRANSFER_PORT)) {
        Ok(l) => l,
        Err(e) => {
            if let Ok(mut s) = state.lock() { s.error = Some(format!("Cannot listen: {}", e)); }
            return;
        }
    };
    for stream in listener.incoming().flatten() {
        ...
        let sd  = save_dir.clone();
        let sa  = Arc::clone(&state); // for progress updates
        thread::spawn(move || match receive_file_resumable(stream, &sd, sa) {
            Ok(f) => {
                let ts = SystemTime::now().duration_since(UNIX_EPOCH).unwrap_or_default().as_secs();
                append_history(&HistoryEntry {
                    timestamp: ts, direction: TransferDir::Received, transfer_type: TransferType::Local,
                    file_name: f.name.clone(), file_size: f.size, peer_name: f.peer_name.clone(),
                    success: true, error: None, file_path: Some(f.path.clone()),
                });
                if let Ok(mut s) = s2.lock() {
                    ...
                    s.active_incoming = None; // clear when done
                    if auto_open { open_folder(&f.path); }
                    s.files.push(f);
                }
            }
            Err(e) if e.starts_with("__skip__") => {
                // MAGIC_SKIP from receiver — file already up to date
                if let Ok(mut s) = s2.lock() { s.active_incoming = None; }
            }
            Err(e) => {
                if let Ok(mut s) = s2.lock() { s.active_incoming = None; s.error = Some(e); }
            }
        });
    }
}
...
fn discovery_responder(_: Arc<Mutex<RecvState>>) {
    let Ok(sock) = UdpSocket::bind(("0.0.0.0", DISCOVER_PORT)) else { return; };
    let name = hostname();
    let mut buf = [0u8; 512];
    loop {
...
fn encrypt_chunk(cipher: &Aes256Gcm, plaintext: &[u8]) -> Result<Vec<u8>, String> {
    ...
    let ct    = cipher.encrypt(nonce, plaintext).map_err(|e| format!("Enc: {}", e))?;
...
fn decrypt_chunk(cipher: &Aes256Gcm, data: &[u8]) -> Result<Vec<u8>, String> {
    if data.len() < AES_NONCE_LEN + AES_TAG_LEN { return Err("Chunk too short".into()); }
    let nonce = Nonce::from_slice(&data[..AES_NONCE_LEN]);
    cipher.decrypt(nonce, &data[AES_NONCE_LEN..])
...
fn write_encrypted(stream: &mut TcpStream, cipher: &Aes256Gcm, pt: &[u8]) -> Result<(), String> {
    let enc = encrypt_chunk(cipher, pt)?;
    stream.write_all(&(enc.len() as u32).to_le_bytes()).map_err(|e| e.to_string())?;
...
fn read_encrypted(stream: &mut TcpStream, cipher: &Aes256Gcm) -> Result<Vec<u8>, String> {
    ...
    let len = u32::from_le_bytes(lb) as usize;
    if len > CHUNK_SIZE + AES_TAG_LEN + AES_NONCE_LEN + 64 { return Err(format!("Packet too large: {}", len)); }
    let mut enc = vec![0u8; len];
    stream.read_exact(&mut enc).map_err(|e| e.to_string())?;
    decrypt_chunk(cipher, &enc)
}

// ─── File transfer — sync folder helper ──────────────────────────────────────
fn send_file_sync(
    path: &std::path::Path,
    name: &str,
    file_size: u64,
    ...
    let mut stream = TcpStream::connect((addr, TRANSFER_PORT)).map_err(|e| format!("Cannot connect: {}", e))?;

    let sender_secret = EphemeralSecret::random_from_rng(AeadOsRng);
    let sender_pub    = PublicKey::from(&sender_secret);
    stream.write_all(sender_pub.as_bytes()).map_err(|e| e.to_string())?;
    stream.flush().map_err(|e| e.to_string())?;
    let mut recv_pub_bytes = [0u8; X25519_KEY_LEN];
    stream.read_exact(&mut recv_pub_bytes).map_err(|e| e.to_string())?;
    ...
    if resp_buf[0] == MAGIC_SKIP  { return Ok(SyncResult::Skipped); }
    if resp_buf[0] != MAGIC_RESUME { return Err("Bad handshake response".into()); }
    ...
    if resume_offset > 0 { file.seek(SeekFrom::Start(resume_offset)).map_err(|e| e.to_string())?; }
...
// ─── File transfer — main send (WiFi TCP with Bluetooth fallback) ────────────
/// Send a file to `addr` over TCP.
/// `batch_info` = `Some((file_index_1based, total_files, total_bytes))` for multi-file batches.
/// `peer_bt_addr` = Bluetooth MAC to fall back to if TCP fails.
fn send_file_resumable(
    ...
    peer_bt_addr: Option<String>,
    on_progress: impl Fn(f32) + Send + 'static,
) -> Result<(), String> {
    // Try TCP first
    match send_file_resumable_tcp(path, name, file_size, addr, batch_info, &on_progress) {
        Ok(()) => return Ok(()),
        Err(e) => {
            // If TCP failed and we have a Bluetooth address, try Bluetooth
            if let Some(ref bt_addr) = peer_bt_addr {
                eprintln!("TCP failed ({}), retrying via Bluetooth {}", e, bt_addr);
                return send_file_resumable_bluetooth(path, name, file_size, bt_addr, batch_info, &on_progress);
            }
            return Err(e);
        }
    }
...
fn send_file_resumable_tcp(
    ...
    let mut stream  = TcpStream::connect_timeout(&socket_addr, std::time::Duration::from_secs(5))
    ...
    // Build OFFER message
    // Layout: [MAGIC_OFFER][name_len:4][name][file_size:8][hn_len:4][hostname]
    //          [batch_total_files:4][batch_file_index:4][batch_total_bytes:8]   (16 bytes, optional)
    let nb = name.as_bytes();
    let hn = hostname();
    let hb = hn.as_bytes();
    ...
    offer.extend_from_slice(&(hb.len() as u32).to_le_bytes());
    offer.extend_from_slice(hb);
    // Append batch info (16 bytes) — receivers that don't understand it ignore the tail
    ...
    stream.read_exact(&mut magic_buf).map_err(|e| e.to_string())?;
    ...
    loop {
        ...
        write_encrypted(&mut stream, &cipher, &buf[..n])?;
        sent += n as u64;
        if file_size > 0 { on_progress(sent as f32 / file_size as f32); }
    }
    stream.write_all(&[MAGIC_DONE]).map_err(|e| e.to_string())?;
    stream.flush().map_err(|e| e.to_string())?;
    Ok(())
...
/// Bluetooth fallback for send — platform-specific.
fn send_file_resumable_bluetooth(
    path: &std::path::Path,
    name: &str,
    file_size: u64,
    bt_addr: &str,
...
fn send_file_via_bluetooth(
    ...
    unsafe {
        ...
        let sock = unsafe { socket(AF_BTH as i32, SOCK_STREAM as i32, BTHPROTO_RFCOMM as i32) };

        if sock == INVALID_SOCKET {
            // Handle error (e.g., check WSAGetLastError())
            return Err("Failed to create Bluetooth socket".to_string());
        }
...
/// Receive one file from `stream`, updating `state.active_incoming` as data arrives.
/// The batch info appended to the OFFER (total_files, file_index, total_bytes) is parsed
/// and surfaced through `IncomingTransfer` for the UI.
fn receive_file_resumable(
    mut stream: TcpStream,
    save_dir: &std::path::Path,
    state: Arc<Mutex<RecvState>>,
) -> Result<ReceivedFile, String> {
    let sender_ip = stream.peer_addr()
        .map(|a| a.ip().to_string()).unwrap_or_else(|_| "unknown".to_string());

    // Key exchange
    let mut sender_pub_bytes = [0u8; X25519_KEY_LEN];
    stream.read_exact(&mut sender_pub_bytes).map_err(|e| e.to_string())?;
    ...
    stream.write_all(receiver_pub.as_bytes()).map_err(|e| e.to_string())?;
    stream.flush().map_err(|e| e.to_string())?;
    ...
    if offer.is_empty() || offer[0] != MAGIC_OFFER { return Err("Bad offer".into()); }
    if offer.len() < 5 { return Err("Offer too short".into()); }

    let name_len = u32::from_le_bytes(offer[1..5].try_into().unwrap()) as usize;
    if name_len == 0 || name_len > 4096 { return Err("Bad name length".into()); }
    if offer.len() < 5 + name_len + 8   { return Err("Truncated offer".into()); }
    ...
    if file_size > 8 * 1024 * 1024 * 1024 { return Err("File too large (>8 GB)".into()); }

    let base = 5 + name_len + 8;

    // Parse sender hostname
    let sender_hostname: String = if offer.len() >= base + 4 {
        ...
        } else { sender_ip.clone() }
    } else { sender_ip.clone() };

    // Parse optional mtime (for sync skip logic)
    let hn_len_val = if offer.len() >= base + 4 {
        u32::from_le_bytes(offer[base..base + 4].try_into().unwrap()) as usize
    } else { 0 };
    let mtime_base  = base + 4 + hn_len_val.min(256);
    let sender_mtime: Option<u64> = if offer.len() >= mtime_base + 8 {
        Some(u64::from_le_bytes(offer[mtime_base..mtime_base + 8].try_into().unwrap()))
    } else { None };

    // Parse optional batch info (16 bytes after mtime)
    // Layout: [total_files:4][file_index:4][total_bytes:8]
    let batch_base = mtime_base + if sender_mtime.is_some() { 8 } else { 0 };
    let (batch_total_files, batch_file_index, batch_total_bytes) =
        if offer.len() >= batch_base + 16 {
            let tf  = u32::from_le_bytes(offer[batch_base..batch_base + 4].try_into().unwrap());
            let fi  = u32::from_le_bytes(offer[batch_base + 4..batch_base + 8].try_into().unwrap());
            let tb  = u64::from_le_bytes(offer[batch_base + 8..batch_base + 16].try_into().unwrap());
            (tf, fi, tb)
        } else { (0, 1, file_size) };
    ...
    } else { (dest, resume_offset) };

    // Send RESUME response
    stream.write_all(&[MAGIC_RESUME]).map_err(|e| e.to_string())?;
    write_encrypted(&mut stream, &cipher, &resume_offset.to_le_bytes())?;
    stream.flush().map_err(|e| e.to_string())?;

    // Set active_incoming so the UI can display progress
    if let Ok(mut s) = state.try_lock() {
        s.active_incoming = Some(IncomingTransfer {
            file_name:         name.clone(),
            file_size,
            received_bytes:    resume_offset,
            peer_name:         sender_hostname.clone(),
            batch_file_index,
            batch_total_files,
            batch_total_bytes,
        });
    ...
    let mut file = fs::OpenOptions::new()
        ...
        .append(resume_offset > 0).truncate(resume_offset == 0)
        .open(&dest).map_err(|e| format!("Cannot open dest: {}", e))?;
    ...
    loop {
        ...
        match magic[0] {
            MAGIC_DATA => {
                ...
                // Update live progress (non-blocking)
                if let Ok(mut s) = state.try_lock() {
                    if let Some(ref mut inc) = s.active_incoming {
                        inc.received_bytes = received;
                    }
                }
            }
            MAGIC_DONE => break,
...
fn send_file_via_relay(
    ...
    relay_stream.set_read_timeout(Some(std::time::Duration::from_secs(300))).map_err(|e| format!("Timeout set: {}", e))?;
    relay_stream.set_write_timeout(Some(std::time::Duration::from_secs(300))).map_err(|e| format!("Timeout set: {}", e))?;

    let sender_secret = EphemeralSecret::random_from_rng(AeadOsRng);
    let sender_pub    = PublicKey::from(&sender_secret);
    relay_stream.write_all(sender_pub.as_bytes()).map_err(|e| e.to_string())?;
    relay_stream.flush().map_err(|e| e.to_string())?;
    let mut recv_pub_bytes = [0u8; X25519_KEY_LEN];
    relay_stream.read_exact(&mut recv_pub_bytes).map_err(|e| e.to_string())?;
    let receiver_pub = PublicKey::from(recv_pub_bytes);
    let shared       = sender_secret.diffie_hellman(&receiver_pub);
    let aes_key      = derive_key(shared.as_bytes());
    let cipher       = Aes256Gcm::new(Key::<Aes256Gcm>::from_slice(&aes_key));
    ...
    let mut magic_buf = [0u8; 1];
    relay_stream.read_exact(&mut magic_buf).map_err(|e| e.to_string())?;
    if magic_buf[0] != MAGIC_RESUME { return Err("Bad handshake response".into()); }
...
fn receive_file_via_relay(
    ...
    let sender_ip = relay_stream.peer_addr()
        .map(|a| a.ip().to_string()).unwrap_or_else(|_| "relay".to_string());

    let mut sender_pub_bytes = [0u8; X25519_KEY_LEN];
    relay_stream.read_exact(&mut sender_pub_bytes).map_err(|e| e.to_string())?;
    let sender_pub     = PublicKey::from(sender_pub_bytes);
    let receiver_secret = EphemeralSecret::random_from_rng(AeadOsRng);
    let receiver_pub   = PublicKey::from(&receiver_secret);
    relay_stream.write_all(receiver_pub.as_bytes()).map_err(|e| e.to_string())?;
    relay_stream.flush().map_err(|e| e.to_string())?;
    let shared   = receiver_secret.diffie_hellman(&sender_pub);
    let aes_key  = derive_key(shared.as_bytes());
    let cipher   = Aes256Gcm::new(Key::<Aes256Gcm>::from_slice(&aes_key));
    ...
    relay_stream.write_all(&[MAGIC_RESUME]).map_err(|e| e.to_string())?;
    ...
    if received != file_size { return Err(format!("Size mismatch: {} vs {}", file_size, received)); }
